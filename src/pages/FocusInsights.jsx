...
export default function FocusInsights() {
  ...
  assignments.forEach((a) => {
    if (!byClass[a.class_name])
      byClass[a.class_name] = { total: 0, done: 0, minutes: 0 };
  ...
  const pieData = classData
    .map((d) => ({ name: d.name, value: d.minutes }))
    .filter((d) => d.value > 0);
      ...
      {assignments.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-muted-foreground text-sm">
            Add some assignments to see your focus breakdown!
          </p>
        </div>
      ) : (
        <>
